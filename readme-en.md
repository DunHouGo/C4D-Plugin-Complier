...
- Preferences contains SDK setup, appearance, and about pages. After choosing Simplified Chinese in Appearance, the main workbench, SDK Sources, Output Preview, buttons, statuses, and primary help text switch immediately.
...
- Save: writes the current SDK root to `sdk_sources.json` in the user config directory. When no preference has been saved, the app uses the current system Documents folder. The legacy repository `configs/sdk_sources.json` is only read if it exists from an older checkout and is not written at runtime.
...
- The About page shows the current app version used by automatic updates to decide whether a newer release should be installed.
- Check for Updates manually checks the GitHub Release updater manifest; when a newer version is available, the app can download, install, and restart.
- Open Downloads opens the GitHub Release page so users can manually download an installer.
- Open GitHub opens the project page for source code, release history, and issue tracking.
...
- Windows legacy SDK builds call Maxon's `projecttool` and the target `.vcxproj` directly, so you do not need to run `generate_solution_win.bat` as administrator.
