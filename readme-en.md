...
- SDK Matrix: available SDK versions. Extracted SDK roots and local SDK archives are treated as buildable sources and shown as ready. Versions with only an official download URL stay visible for one-click setup and build-time downloads. Installed Cinema 4D `sdk.zip` files are only a compatibility fallback when no official extension SDK URL is known. After One-click Setup or saving the SDK root, the workbench refreshes the version list automatically, so greyed-out versions usually become selectable immediately.
...
- Package: release package and output folder name. The SDK module name stays aligned with the official plugin module layout: `sdk_custom_paths.txt` can provide a module alias such as `postwatermark`, direct plugin roots use their module folder name, and a single nested SDK module such as `BackHighlight/draw.back/project/projectdefinition.txt` is used as the actual build target. The 2024.4 legacy SDK preserves the complete nested path when generating and locating its Xcode project. Packaged binaries keep the official module filename, for example `postwatermark.xdl64`.
...
- Package Mode: packaging mode, one of `Merged`, `Per Version`, or `Both`. Merged packages keep one output folder and place each version/configuration binary directly inside it, while Per Version creates separate top-level folders per version.
- Artifact naming: package folders keep only the C4D major version, so `2024.4` outputs as `2024`; Release has no configuration suffix, while Debug adds `_Debug`. Binary filenames keep the SDK module name generated by Maxon's build system.
